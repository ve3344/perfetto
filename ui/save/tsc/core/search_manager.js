...
exports.SearchManagerImpl = void 0;
...
class SearchManagerImpl {
    ...
    stepInternal(reverse = false) {
        if (this._searchWindow === undefined)
            return;
        ...
        if (index === -1 ||
            ...
            if (reverse) {
                const [smaller] = (0, binary_search_1.searchSegment)(this._results.tses, endNs);
                ...
                const [, larger] = (0, binary_search_1.searchSegment)(this._results.tses, startNs);
        ...
        if (this._onResultStep) {
            this._onResultStep({
                ...
                ts: time_1.Time.fromRaw(this._results.tses[this._resultIndex]),
    ...
    setResultIndexWithSaturation(nextIndex) {
        const tot = (0, logging_1.assertExists)(this._results).totalResults;
        (0, logging_1.assertTrue)(tot !== 0); // The early out in step() guarantees this.
      ...
      from (
        select
          ...
          where slice.name glob ${searchLiteral}
            or (
              0 != CAST(${(0, string_utils_1.sqliteString)(search)} AS INT) and
              sliceId = CAST(${(0, string_utils_1.sqliteString)(search)} AS INT)
            )
        union
        select
          slice_id as sliceId,
    ...
    `);
        ...
        searchResults.eventIds = new Float64Array(searchResults.totalResults + rows);
        ...
        const it = res.iter({
            sliceId: query_result_1.NUM,
            ts: query_result_1.LONG,
            source: query_result_1.STR,
            sourceId: query_result_1.NUM,
        ...
        for (; it.valid(); it.next()) {
            ...
            if (it.source === 'cpu') {
                ...
                track = trackManager
                    .getAllTracks()
                    .find((td) => td.tags?.kind === track_kinds_1.ANDROID_LOGS_TRACK_KIND)?.uri;
            }
...
//# sourceMappingURL=search_manager.js.map