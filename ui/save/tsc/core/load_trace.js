...
const cache_manager_1 = require("./cache_manager");
...
const trace_stream_1 = require("../core/trace_stream");
const state_serialization_1 = require("./state_serialization");
const raf_scheduler_1 = require("./raf_scheduler");
const trace_impl_1 = require("./trace_impl");
const router_1 = require("../core/router");
const ENABLE_CHROME_RELIABLE_RANGE_ZOOM_FLAG = feature_flags_1.featureFlags.register({
...
const ENABLE_CHROME_RELIABLE_RANGE_ANNOTATION_FLAG = feature_flags_1.featureFlags.register({
...
const CROP_TRACK_EVENTS_FLAG = feature_flags_1.featureFlags.register({
    id: 'cropTrackEvents',
    name: 'Crop track events',
    description: 'Ignores track events outside of the range of interest',
    defaultValue: false,
});
const INGEST_FTRACE_IN_RAW_TABLE_FLAG = feature_flags_1.featureFlags.register({
...
const ANALYZE_TRACE_PROTO_CONTENT_FLAG = feature_flags_1.featureFlags.register({
    id: 'analyzeTraceProtoContent',
    name: 'Analyze trace proto content',
    description: 'Enables trace proto content analysis (experimental_proto_content table)',
    defaultValue: false,
});
const FTRACE_DROP_UNTIL_FLAG = feature_flags_1.featureFlags.register({
    id: 'ftraceDropUntilAllCpusValid',
    name: 'Crop ftrace events',
    description: 'Drop ftrace events until all per-cpu data streams are known to be valid',
    defaultValue: true,
});
// TODO(stevegolton): Move this into some global "SQL extensions" file and
// ensure it's only run once.
async function defineMaxLayoutDepthSqlFunction(engine) {
    await engine.query(`
...
let lastEngineId = 0;
async function loadTrace(app, traceSource) {
...
async function createEngine(app, engineId) {
    ...
    if (app.httpRpc.newEngineMode === 'USE_HTTP_RPC_IF_AVAILABLE') {
        useRpc = (await http_rpc_engine_1.HttpRpcEngine.checkConnection()).connected;
    }
    let engine;
    if (useRpc) {
        console.log('Opening trace using native accelerator over HTTP+RPC');
        engine = new http_rpc_engine_1.HttpRpcEngine(engineId);
    }
    else {
    ...
    engine.onResponseReceived = () => raf_scheduler_1.raf.scheduleFullRedraw('force');
    if ((0, metatracing_1.isMetatracingEnabled)()) {
...
async function loadTraceIntoEngine(app, traceSource, engine) {
    ...
    if (traceSource.type === 'FILE') {
        ...
        traceStream = new trace_stream_1.TraceBufferStream(traceSource.buffer);
    }
    ...
    if (traceStream !== undefined) {
        ...
        (0, logging_1.assertTrue)(engine instanceof http_rpc_engine_1.HttpRpcEngine);
    ...
    router_1.Router.navigate(`#!/viewer?local_cache_key=${cacheUuid}`);
    // Make sure the helper views are available before we start adding tracks.
    await includeSummaryTables(trace);
    await defineMaxLayoutDepthSqlFunction(engine);
    if (serializedAppState !== undefined) {
        (0, state_serialization_1.deserializeAppStatePhase1)(serializedAppState, trace);
    }
...
async function includeSummaryTables(trace) {
...
function updateStatus(traceOrApp, msg) {
    const showUntilDismissed = 0;
    traceOrApp.omnibox.showStatusMessage(msg, showUntilDismissed);
}
async function computeFtraceBounds(engine) {
    const result = await engine.query(`
    SELECT min(ts) as start, max(ts) as end FROM ftrace_event;
  `);
    const { start, end } = result.firstRow({ start: query_result_1.LONG_NULL, end: query_result_1.LONG_NULL });
    if (start !== null && end !== null) {
        return new time_1.TimeSpan(time_1.Time.fromRaw(start), time_1.Time.fromRaw(end));
    }
    return null;
}
async function computeTraceReliableRangeStart(engine) {
    const result = await engine.query(`SELECT RUN_METRIC('chrome/chrome_reliable_range.sql');
       SELECT start FROM chrome_reliable_range`);
    const bounds = result.firstRow({ start: query_result_1.LONG });
    return time_1.Time.fromRaw(bounds.start);
}
async function computeVisibleTime(traceStart, traceEnd, isJsonTrace, engine) {
    ...
    if (time_1.Time.max(visibleStart, mdTime.start) < time_1.Time.min(visibleEnd, mdTime.end)) {
        visibleStart = time_1.Time.max(visibleStart, mdTime.start);
        visibleEnd = time_1.Time.min(visibleEnd, mdTime.end);
    }
    // Trace Processor doesn't support the reliable range feature for JSON
    // traces.
    if (!isJsonTrace && ENABLE_CHROME_RELIABLE_RANGE_ZOOM_FLAG.get()) {
        const reliableRangeStart = await computeTraceReliableRangeStart(engine);
        visibleStart = time_1.Time.max(visibleStart, reliableRangeStart);
    }
    ...
    if (ftraceBounds !== null) {
        ...
        visibleStart = time_1.Time.min(ftraceBounds.start, visibleEnd);
    }
...
async function getTraceInfo(engine, traceSource) {
    ...
    const it = result.iter({
        ...
        clockValue: query_result_1.LONG,
        clockName: query_result_1.STR,
    });
    let snapshot = {
        clockName: '',
        ts: time_1.Time.ZERO,
        clockValue: time_1.Time.ZERO,
    };
    ...
    for (let row = 0; it.valid(); it.next(), row++) {
        if (it.clockName === 'REALTIME') {
            snapshot = {
                ...
                clockValue: time_1.Time.fromRaw(it.clockValue),
            };
            break;
        }
    ...
    const resTz = await (0, logging_1.assertExists)(engine).query(queryTz);
    const tzOffMin = resTz.firstRow({ tzOffMin: query_result_1.NUM_NULL }).tzOffMin ?? 0;
    ...
    switch (traceSource.type) {
        ...
        case 'ARRAY_BUFFER':
            ...
            break;
        case 'HTTP_RPC':
            traceTitle = `RPC @ ${http_rpc_engine_1.HttpRpcEngine.hostAndPort}`;
            break;
        default:
            break;
    }
    const traceType = await getTraceType(engine);
    const hasFtrace = (await engine.query(`select * from ftrace_event limit 1`)).numRows() > 0;
...
async function getTraceType(engine) {
    const result = await engine.query(`select str_value from metadata where name = 'trace_type'`);
    if (result.numRows() === 0)
        return undefined;
    return result.firstRow({ str_value: query_result_1.STR }).str_value;
}
async function getTraceTimeBounds(engine) {
    const result = await engine.query(`select start_ts as startTs, end_ts as endTs from trace_bounds`);
    const bounds = result.firstRow({
        startTs: query_result_1.LONG,
        endTs: query_result_1.LONG,
    });
    return new time_1.TimeSpan(time_1.Time.fromRaw(bounds.startTs), time_1.Time.fromRaw(bounds.endTs));
}
// TODO(hjd): When streaming must invalidate this somehow.
async function getCpus(engine) {
    const cpus = [];
    const queryRes = await engine.query('select distinct(cpu) as cpu from sched order by cpu;');
    for (const it = queryRes.iter({ cpu: query_result_1.NUM }); it.valid(); it.next()) {
        cpus.push(it.cpu);
    }
    return cpus;
}
async function getTraceErrors(engine) {
    const sql = `SELECT sum(value) as errs FROM stats WHERE severity != 'info'`;
    const result = await engine.query(sql);
    return result.firstRow({ errs: query_result_1.NUM }).errs;
}
async function getTracingMetadataTimeBounds(engine) {
    ...
    let startBound = time_1.Time.MIN;
    let endBound = time_1.Time.MAX;
    const it = queryRes.iter({ name: query_result_1.STR, intValue: query_result_1.LONG_NULL });
    for (; it.valid(); it.next()) {
        ...
        if (columnName === 'tracing_disabled_ns') {
            endBound = time_1.Time.min(endBound, time_1.Time.fromRaw(timestamp));
        }
        else {
            startBound = time_1.Time.max(startBound, time_1.Time.fromRaw(timestamp));
        }
    }
    return new time_1.TimeSpan(startBound, endBound);
}
//# sourceMappingURL=load_trace.js.map