...
class ScrollHelper {
    ...
    workspace;
    trackManager;
    constructor(traceInfo, timeline, workspace, trackManager) {
    ...
    scrollTo(args) {
        ...
        if (time !== undefined) {
            if (time.end === undefined) {
                ...
                this.focusHorizontalRangePercentage(time.start, time.end, time.viewPercentage);
            }
            else {
    ...
    focusHorizontalRangePercentage(start, end, viewPercentage) {
        const aoi = high_precision_time_span_1.HighPrecisionTimeSpan.fromTime(start, end);
    ...
    focusHorizontalRange(start, end) {
        ...
        if (aoi.duration * 2 > visible.duration) {
            ...
            const newVisibleWindow = new high_precision_time_span_1.HighPrecisionTimeSpan(newStart, visible.duration).fitWithin(this.traceInfo.start, this.traceInfo.end);
