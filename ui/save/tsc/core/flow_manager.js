...
const SHOW_INDIRECT_PRECEDING_FLOWS_FLAG = feature_flags_1.featureFlags.register({
...
class FlowManager {
    ...
    _selectedFlows = [];
    _curSelection;
    _focusedFlowIdLeft = -1;
    _focusedFlowIdRight = -1;
    _visibleCategories = new Map();
    _initialized = false;
    constructor(engine, trackMgr, selectionMgr) {
        this.engine = engine;
    ...
    async queryFlowEvents(query) {
        const result = await this.engine.query(query);
        const flows = [];
        const it = result.iter({
            beginSliceId: query_result_1.NUM,
            beginTrackId: query_result_1.NUM,
            beginSliceName: query_result_1.STR_NULL,
            beginSliceChromeCustomName: query_result_1.STR_NULL,
            beginSliceCategory: query_result_1.STR_NULL,
            beginSliceStartTs: query_result_1.LONG,
            beginSliceEndTs: query_result_1.LONG,
            beginDepth: query_result_1.NUM,
            beginThreadName: query_result_1.STR_NULL,
            beginProcessName: query_result_1.STR_NULL,
            endSliceId: query_result_1.NUM,
            endTrackId: query_result_1.NUM,
            endSliceName: query_result_1.STR_NULL,
            endSliceChromeCustomName: query_result_1.STR_NULL,
            endSliceCategory: query_result_1.STR_NULL,
            endSliceStartTs: query_result_1.LONG,
            endSliceEndTs: query_result_1.LONG,
            endDepth: query_result_1.NUM,
            endThreadName: query_result_1.STR_NULL,
            endProcessName: query_result_1.STR_NULL,
            name: query_result_1.STR_NULL,
            category: query_result_1.STR_NULL,
            id: query_result_1.NUM,
            flowToDescendant: query_result_1.NUM,
        });
        const nullToStr = (s) => {
            return s === null ? 'NULL' : s;
        };
        const nullToUndefined = (s) => {
        ...
        for (; it.valid(); it.next()) {
            ...
            const begin = {
                ...
                sliceId: (0, core_types_1.asSliceSqlId)(it.beginSliceId),
                ...
                sliceStartTs: time_1.Time.fromRaw(it.beginSliceStartTs),
                sliceEndTs: time_1.Time.fromRaw(it.beginSliceEndTs),
                depth: it.beginDepth,
            ...
            const end = {
                ...
                sliceId: (0, core_types_1.asSliceSqlId)(it.endSliceId),
                ...
                sliceStartTs: time_1.Time.fromRaw(it.endSliceStartTs),
                sliceEndTs: time_1.Time.fromRaw(it.endSliceEndTs),
                depth: it.endDepth,
        ...
        const trackUriToInfo = new Map();
        const trackIdToInfo = new Map();
        const trackIdToTrack = new Map();
        this.trackMgr
            .getAllTracks()
            .forEach((trackDescriptor) => trackDescriptor.tags?.trackIds?.forEach((trackId) => trackIdToTrack.set(trackId, trackDescriptor)));
        const getInfo = (trackId) => {
    ...
    areaSelected(area) {
        ...
        for (const trackInfo of area.tracks) {
            ...
            if (kind === track_kinds_1.SLICE_TRACK_KIND ||
    ...
    setConnectedFlows(connectedFlows) {
    ...
    focusOtherFlow(direction) {
        ...
        const boundFlows = this._connectedFlows.filter((flow) => (flow.begin.sliceId === sliceId && direction === 'Forward') ||
            (flow.end.sliceId === sliceId && direction === 'Backward'));
        if (direction === 'Backward') {
            const nextFlowId = findAnotherFlowExcept(boundFlows, this._focusedFlowIdLeft);
            this._focusedFlowIdLeft = nextFlowId;
        }
        else {
            const nextFlowId = findAnotherFlowExcept(boundFlows, this._focusedFlowIdRight);
            this._focusedFlowIdRight = nextFlowId;
        }
        raf_scheduler_1.raf.scheduleFullRedraw();
    }
    // Select the slice connected to the flow in focus
    moveByFocusedFlow(direction) {
        const currentSelection = this._curSelection;
        if (!currentSelection || currentSelection.kind !== 'track_event') {
            return;
        }
        const sliceId = currentSelection.eventId;
        const flowId = direction === 'Backward'
...
//# sourceMappingURL=flow_manager.js.map