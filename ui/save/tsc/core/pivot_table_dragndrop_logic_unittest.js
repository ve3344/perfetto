    ...
    test('has the same elements in the result', () => {
        ...
        for (let i = 0; i < arr.length; i++) {
            for (let j = 0; j < arr.length; j++) {
                ...
                const permutedLeft = (0, pivot_table_manager_1.performReordering)((0, pivot_table_manager_1.computeIntervals)(arr.length, i, j, 'left'), arr);
                expect(new Set(permutedLeft)).toEqual(arrSet);
                expect(permutedLeft.length).toEqual(arr.length);
                const permutedRight = (0, pivot_table_manager_1.performReordering)((0, pivot_table_manager_1.computeIntervals)(arr.length, i, j, 'right'), arr);
...
//# sourceMappingURL=pivot_table_dragndrop_logic_unittest.js.map