...
exports.addVisualizedArgTracks = addVisualizedArgTracks;
const uuid_1 = require("../../base/uuid");
const query_result_1 = require("../../trace_processor/query_result");
const visualized_args_track_1 = require("./visualized_args_track");
...
async function addVisualizedArgTracks(trace, argName) {
    ...
    `);
    const addedTracks = [];
    const it = result.iter({ trackId: query_result_1.NUM, maxDepth: query_result_1.NUM });
    for (; it.valid(); it.next()) {
        const trackId = it.trackId;
        const maxDepth = it.maxDepth;
        const uri = `${VISUALIZED_ARGS_SLICE_TRACK_URI_PREFIX}#${(0, uuid_1.uuidv4)()}`;
        trace.tracks.registerTrack({
            uri,
            title: argName,
            chips: ['arg'],
            track: new visualized_args_track_1.VisualizedArgsTrack({
                trace,
                uri,
                trackId,
                maxDepth,
        ...
        const threadSliceTrack = trace.workspace.flatTracks.find((trackNode) => {
            ...
            return (trackDescriptor &&
                trackDescriptor.tags?.kind === track_kinds_1.SLICE_TRACK_KIND &&
                trackDescriptor.tags?.trackIds?.includes(trackId));
        });
        const parentGroup = threadSliceTrack?.parent;
        if (parentGroup) {
            const newTrack = new workspace_1.TrackNode({ uri, title: argName });
            parentGroup.addChildBefore(newTrack, threadSliceTrack);
            addedTracks.push(newTrack);
        }
    }
}
//# sourceMappingURL=visualized_args_tracks.js.map