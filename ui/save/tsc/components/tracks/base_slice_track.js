...
exports.BaseSliceTrack = exports.BASE_ROW = exports.filterVisibleSlicesForTesting = exports.FADE_THIN_SLICES_FLAG = exports.CROP_INCOMPLETE_SLICE_FLAG = exports.SLICE_FLAGS_INSTANT = exports.SLICE_FLAGS_INCOMPLETE = void 0;
...
const SLICE_MIN_WIDTH_PX = 1 / timeline_cache_1.BUCKETS_PER_PIXEL;
const SLICE_MIN_WIDTH_FADED_PX = 0.1;
const CHEVRON_WIDTH_PX = 10;
const DEFAULT_SLICE_COLOR = colorizer_1.UNEXPECTED_PINK;
const INCOMPLETE_SLICE_WIDTH_PX = 20;
exports.CROP_INCOMPLETE_SLICE_FLAG = feature_flags_1.featureFlags.register({
    id: 'cropIncompleteSlice',
    name: 'Crop incomplete slices',
    description: 'Display incomplete slices in short form',
    defaultValue: false,
});
exports.FADE_THIN_SLICES_FLAG = feature_flags_1.featureFlags.register({
    id: 'fadeThinSlices',
...
function filterVisibleSlices(slices, start, end) {
    ...
    if ((0, utils_1.exists)(maybeFirstSlice) && maybeFirstSlice.startNs > end) {
        return [];
    }
...
exports.BASE_ROW = {
    id: query_result_1.NUM, // The slice ID, for selection / lookups.
    ts: query_result_1.LONG, // True ts in nanoseconds.
    dur: query_result_1.LONG, // True duration in nanoseconds. -1 = incomplete, 0 = instant.
    tsQ: query_result_1.LONG, // Quantized start time in nanoseconds.
    durQ: query_result_1.LONG, // Quantized duration in nanoseconds.
    depth: query_result_1.NUM, // Vertical depth.
};
class BaseSliceTrack {
    trace;
    uri;
    sliceLayout = { ...slice_layout_1.DEFAULT_SLICE_LAYOUT };
    trackUuid = (0, uuid_1.uuidv4Sql)();
    // This is the over-skirted cached bounds:
    slicesKey = timeline_cache_1.CacheKey.zero();
    ...
    maxDataDepth = 0;
    // Computed layout.
    computedTrackHeight = 0;
    computedSliceHeight = 0;
    computedRowSpacing = 0;
    trash;
    // Extension points.
    ...
    constructor(trace, uri) {
        ...
        const baseCols = Object.keys(exports.BASE_ROW);
    ...
    setSliceLayout(sliceLayout) {
        if (sliceLayout.isFlat &&
            sliceLayout.depthGuess !== undefined &&
            sliceLayout.depthGuess !== 0) {
            const { isFlat, depthGuess } = sliceLayout;
            throw new Error(`if isFlat (${isFlat}) then depthGuess (${depthGuess}) must be 0 if defined`);
        }
    ...
    async onCreate() {
        ...
        let queryRes;
        if (exports.CROP_INCOMPLETE_SLICE_FLAG.get()) {
      ...
      `);
        ...
        const incomplete = new Array(queryRes.numRows());
    ...
    async onUpdate({ visibleWindow, size }) {
        ...
        const rawSlicesKey = timeline_cache_1.CacheKey.create(timespan.start, timespan.end, windowSizePx);
        // If the visible time range is outside the cached area, requests
        // asynchronously new data from the SQL engine.
        await this.maybeRequestData(rawSlicesKey);
    }
    render({ ctx, size, visibleWindow, timescale }) {
        ...
        const vizSlices = this.getVisibleSlicesInternal(visibleWindow.start.toTime('floor'), visibleWindow.end.toTime('ceil'));
        const selection = this.trace.selection.selection;
        const selectedId = selection.kind === 'track_event' && selection.trackUri === this.uri
            ? selection.eventId
            : undefined;
        if (selectedId === undefined) {
            this.selectedSlice = undefined;
        }
        let discoveredSelection;
        ...
        vizSlicesByColor.sort((a, b) => (0, color_1.colorCompare)(a.colorScheme.base, b.colorScheme.base));
        ...
        for (const slice of vizSlicesByColor) {
            ...
            if (slice.flags & exports.SLICE_FLAGS_INSTANT) {
                ...
                const w = exports.CROP_INCOMPLETE_SLICE_FLAG.get()
                    ? slice.w
                    : Math.max(slice.w - 2, 2);
                (0, canvas_utils_1.drawIncompleteSlice)(ctx, slice.x, y, w, sliceHeight, !exports.CROP_INCOMPLETE_SLICE_FLAG.get());
            }
            else {
                const w = Math.max(slice.w, exports.FADE_THIN_SLICES_FLAG.get()
        ...
        for (const slice of vizSlicesByColor) {
            ...
            if (slice.flags & (exports.SLICE_FLAGS_INCOMPLETE | exports.SLICE_FLAGS_INSTANT)) {
                continue;
            }
            // Clamp fillRatio between 0.0 -> 1.0
            const fillRatio = (0, math_utils_1.clamp)(slice.fillRatio, 0, 1);
            // Don't draw anything if the fill ratio is 1.0ish
            if ((0, math_utils_1.floatEqual)(fillRatio, 1)) {
                continue;
            }
        ...
        for (const slice of vizSlices) {
            ...
            const title = (0, string_utils_1.cropText)(slice.title, charWidth, slice.w);
        ...
        for (const slice of vizSlices) {
            ...
            const rectXCenter = slice.x + slice.w / 2;
            const subTitle = (0, string_utils_1.cropText)(slice.subTitle, charWidth, slice.w);
            const y = padding + slice.depth * (sliceHeight + rowSpacing);
        ...
        if (discoveredSelection !== undefined) {
            ...
            ctx.strokeRect(slice.x, y - THICKNESS / 2, slice.w, sliceHeight + THICKNESS);
            ctx.closePath();
        }
        // If the cached trace slices don't fully cover the visible time range,
        // show a gray rectangle with a "Loading..." label.
        (0, checkerboard_1.checkerboardExcept)(ctx, this.getHeight(), 0, size.width, timescale.timeToPx(this.slicesKey.start), timescale.timeToPx(this.slicesKey.end));
        ...
        if (this.hoveredSlice !== undefined &&
            ...
            if (tooltip.length === 1) {
                (0, canvas_utils_1.drawTrackHoverTooltip)(ctx, this.hoverPos, size, tooltip[0]);
            }
            else {
                (0, canvas_utils_1.drawTrackHoverTooltip)(ctx, this.hoverPos, size, tooltip[0], tooltip[1]);
            }
        } // if (hoveredSlice)
    }
    async onDestroy() {
    ...
    // onBoundsChange).
    async maybeRequestData(rawSlicesKey) {
        ...
        if (!rawSlicesKey.isCoveredBy(slicesKey)) {
            throw new Error(`Normalization error ${slicesKey.toString()} ${rawSlicesKey.toString()}`);
        }
    ...
    rowToSliceBase(row) {
        ...
        return {
            id: row.id,
            startNs: time_1.Time.fromRaw(row.tsQ),
            endNs: time_1.Time.fromRaw(row.tsQ + row.durQ),
            durNs: row.durQ,
    ...
    findSlice({ x, y, timescale }) {
        ...
        for (const slice of this.incomplete) {
            const startPx = exports.CROP_INCOMPLETE_SLICE_FLAG.get()
                ? timescale.timeToPx(slice.startNs)
                : slice.x;
            const cropUnfinishedSlicesCondition = exports.CROP_INCOMPLETE_SLICE_FLAG.get()
    ...
    updateHoveredSlice(slice) {
        ...
        if (this.hoveredSlice === undefined) {
            ...
            this.onSliceOut({ slice: (0, logging_1.assertExists)(lastHoveredSlice) });
            this.hoverTooltip = [];
            this.hoverPos = undefined;
        }
        else {
    ...
    getVisibleSlicesInternal(start, end) {
        ...
        start = time_1.Time.sub(start, this.slicesKey.bucketSize);
        end = time_1.Time.add(end, this.slicesKey.bucketSize);
        let slices = filterVisibleSlices(this.slices, start, end);
    ...
    highlightHoveredAndSameTitle(slices) {
        for (const slice of slices) {
            const isHovering = this.trace.timeline.highlightedSliceId === slice.id ||
    ...
    `;
        ...
        return { ts: time_1.Time.fromRaw(row.ts), dur: time_1.Duration.fromRaw(row.dur) };
    }
    getDataset() {
...
//# sourceMappingURL=base_slice_track.js.map