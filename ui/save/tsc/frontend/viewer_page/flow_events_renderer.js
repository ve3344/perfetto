...
const bezier_arrow_1 = require("../../base/canvas/bezier_arrow");
 ...
 */
function renderFlows(trace, ctx, size, panels, trackRoot) {
    const timescale = new time_scale_1.TimeScale(trace.timeline.visibleWindow, {
        left: 0,
        right: size.width,
    });
    ...
    const panelsByTrackNode = new Map(panels.map((panel) => [panel.panel.trackNode, panel]));
    ...
    trace.flows.selectedFlows.forEach((flow) => {
        const categories = (0, flow_events_panel_1.getFlowCategories)(flow);
        for (const cat of categories) {
            if (trace.flows.visibleCategories.get(cat) ||
                trace.flows.visibleCategories.get(flow_events_panel_1.ALL_CATEGORIES)) {
