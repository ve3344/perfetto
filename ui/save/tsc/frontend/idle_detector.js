...
class IdleDetector {
    ...
    idleSince;
    idleHysteresisMs = IDLE_HYSTERESIS_MS;
    waitForPerfettoIdle(idleHysteresisMs = IDLE_HYSTERESIS_MS) {
    ...
    onIdleCallback() {
        ...
        if (now > this.deadline) {
            this.promise.reject(`Didn't reach idle within ${TIMEOUT_MS} ms, giving up` +
                ` ${this.idleIndicators()}`);
            return;
        }
    ...
    idleIndicators() {
        const reqsPending = app_impl_1.AppImpl.instance.trace?.engine.numRequestsPending ?? 0;
        return [
            !app_impl_1.AppImpl.instance.isLoadingTrace,
            reqsPending === 0,
            !raf_scheduler_1.raf.hasPendingRedraws,
...
//# sourceMappingURL=idle_detector.js.map