...
function hptime(time) {
    const array = time.split('.');
    if (array.length > 2)
        throw new Error(`Bad time format ${time}`);
    ...
    if (fractions) {
        ...
        if (negative) {
            return new high_precision_time_1.HighPrecisionTime(t(numBase - 1n), 1.0 - numFractions);
        }
        else {
            return new high_precision_time_1.HighPrecisionTime(t(numBase), numFractions);
        }
    }
    else {
        return new high_precision_time_1.HighPrecisionTime(t(numBase));
    }
}
describe('HighPrecisionTimeSpan', () => {
    it('can be constructed from integer time', () => {
        const span = high_precision_time_span_1.HighPrecisionTimeSpan.fromTime(t(10n), t(20n));
    ...
    test('intersect', () => {
        const span = new high_precision_time_span_1.HighPrecisionTimeSpan(hptime('5'), 3);
    ...
    test('clampDuration', () => {
        const span = new high_precision_time_span_1.HighPrecisionTimeSpan(hptime('5'), 1);
    ...
    test('equality', () => {
        const span = new high_precision_time_span_1.HighPrecisionTimeSpan(hptime('10'), 10);
        expect(span.equals(span)).toBe(true);
        expect(span.equals(new high_precision_time_span_1.HighPrecisionTimeSpan(hptime('10'), 10.5))).toBe(false);
        expect(span.equals(new high_precision_time_span_1.HighPrecisionTimeSpan(hptime('10.1'), 10))).toBe(false);
    });
    test('contains', () => {
        const span = new high_precision_time_span_1.HighPrecisionTimeSpan(hptime('10'), 10);
...
//# sourceMappingURL=high_precision_time_span_unittest.js.map