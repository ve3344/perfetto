...
describe('Vector2D', () => {
    test('add', () => {
        const vector1 = new geom_1.Vector2D({ x: 1, y: 2 });
        const vector2 = new geom_1.Vector2D({ x: 3, y: 4 });
        const result = vector1.add(vector2);
        expect(result.x).toBe(4);
        expect(result.y).toBe(6);
    });
    test('sub', () => {
        const vector1 = new geom_1.Vector2D({ x: 5, y: 7 });
        const vector2 = new geom_1.Vector2D({ x: 2, y: 3 });
        const result = vector1.sub(vector2);
        expect(result.x).toBe(3);
        expect(result.y).toBe(4);
    });
    test('scale', () => {
        const vector = new geom_1.Vector2D({ x: 2, y: 3 });
...
describe('Rect2D', () => {
    test('intersect', () => {
        const a = new geom_1.Rect2D({ left: 1, top: 1, right: 4, bottom: 4 });
    ...
    test('expand', () => {
        const rect = new geom_1.Rect2D({ left: 1, top: 1, right: 3, bottom: 3 });
    ...
    test('reframe', () => {
        const rect = new geom_1.Rect2D({ left: 2, top: 2, right: 5, bottom: 5 });
        const result = rect.reframe({ x: 1, y: 1 });
        expect(result).toMatchObject({ left: 1, top: 1, right: 4, bottom: 4 });
    });
    test('size', () => {
        const rect = new geom_1.Rect2D({ left: 1, top: 1, right: 4, bottom: 3 });
    ...
    it('contains', () => {
        const outerRect = new geom_1.Rect2D({ left: 0, top: 0, right: 10, bottom: 10 });
        const innerRect = { left: 2, top: 2, right: 8, bottom: 8 };
        expect(outerRect.contains(innerRect)).toBe(true);
        const nonContainedRect = { left: 2, top: 2, right: 12, bottom: 8 };
        expect(outerRect.contains(nonContainedRect)).toBe(false);
    });
    test('fromPointAndSize', () => {
        const rect = geom_1.Rect2D.fromPointAndSize({
            x: 10,
            y: 20,
            width: 100,
            height: 50,
    ...
    describe('containsPoint', () => {
        let rect;
        beforeEach(() => {
            rect = new geom_1.Rect2D({ left: 10, top: 20, right: 110, bottom: 70 });
        });
...
//# sourceMappingURL=geom_unittest.js.map