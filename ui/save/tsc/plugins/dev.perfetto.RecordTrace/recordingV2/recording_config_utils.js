...
exports.RecordingConfigUtils = void 0;
exports.genTraceConfig = genTraceConfig;
const object_utils_1 = require("../../../base/object_utils");
const string_utils_1 = require("../../../base/string_utils");
const utils_1 = require("../../../base/utils");
const protos_1 = require("../protos");
var PerfClock = protos_1.PerfEvents.PerfClock;
var Timebase = protos_1.PerfEvents.Timebase;
var CallstackSampling = protos_1.PerfEventConfig.CallstackSampling;
var Scope = protos_1.PerfEventConfig.Scope;
class RecordingConfigUtils {
    lastConfig;
    lastTargetInfo;
    configProtoText;
    configProtoBase64;
    hasDataSources = false;
    fetchLatestRecordCommand(recordConfig, targetInfo) {
        ...
        const configProto = protos_1.TraceConfig.encode(traceConfig).finish();
        this.configProtoText = toPbtxt(configProto);
        this.configProtoBase64 = (0, string_utils_1.base64Encode)(configProto);
...
exports.RecordingConfigUtils = RecordingConfigUtils;
function enableSchedBlockedReason(androidApiLevel) {
    return androidApiLevel !== undefined && androidApiLevel >= 31;
}
function enableCompactSched(androidApiLevel) {
    return androidApiLevel !== undefined && androidApiLevel >= 31;
}
function genTraceConfig(uiCfg, targetInfo) {
    ...
    if (uiCfg.meminfo) {
        ...
        sysStatsCfg.meminfoCounters = uiCfg.meminfoCounters.map((name) => {
            ...
            return protos_1.MeminfoCounters[name];
        });
    }
    if (uiCfg.vmstat) {
        if (sysStatsCfg === undefined)
        ...
        sysStatsCfg.vmstatCounters = uiCfg.vmstatCounters.map((name) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return protos_1.VmstatCounters[name];
        });
    }
    if (uiCfg.memLmk) {
    ...
    if (uiCfg.androidNetworkTracing) {
        if (targetInfo.targetType !== 'CHROME') {
            const net = new protos_1.TraceConfig.DataSource();
            net.config = new protos_1.DataSourceConfig();
            net.config.name = 'android.network_packets';
            net.config.networkPacketTraceConfig = new protos_1.NetworkPacketTraceConfig();
            ...
            const pkg = new protos_1.TraceConfig.DataSource();
            pkg.config = new protos_1.DataSourceConfig();
    ...
    if (uiCfg.androidStatsd) {
        ...
        ds.config.statsdTracingConfig = new protos_1.StatsdTracingConfig();
        ...
        if (uiCfg.androidStatsdPushedAtoms.length > 0) {
            ds.config.statsdTracingConfig.pushAtomId =
                uiCfg.androidStatsdPushedAtoms.map((atom) => atom);
        }
        const needPulledAtomConfig = uiCfg.androidStatsdRawPulledAtoms.length > 0 ||
            uiCfg.androidStatsdPulledAtoms.length > 0;
        if (needPulledAtomConfig) {
            const pullAtomConfig = new protos_1.StatsdPullAtomConfig();
            ...
            pullAtomConfig.pullAtomId = uiCfg.androidStatsdPulledAtoms.map((atom) => atom);
    ...
    if (uiCfg.audio) {
        function addCategoryAndDisabledByDefault(category) {
    ...
    if (uiCfg.tracePerf) {
        ...
        const perfEventConfig = new protos_1.PerfEventConfig();
    ...
    if (chromeCategories.size !== 0) {
        ...
        const chromeConfig = new protos_1.ChromeConfig();
        chromeConfig.clientPriority = protos_1.ChromeConfig.ClientPriority.USER_INITIATED;
        ...
        const trackEventDs = new protos_1.TraceConfig.DataSource();
        trackEventDs.config = new protos_1.DataSourceConfig();
        trackEventDs.config.name = 'track_event';
        trackEventDs.config.chromeConfig = chromeConfig;
        trackEventDs.config.trackEventConfig = new protos_1.TrackEventConfig();
        ...
        if (chromeCategories.has('disabled-by-default-cpu_profiler') ||
            ...
            const dataSource = new protos_1.TraceConfig.DataSource();
            dataSource.config = new protos_1.DataSourceConfig();
    ...
    if (targetInfo.targetType !== 'WINDOWS' &&
    ...
    if (uiCfg.ftrace ||
        ...
        if ((0, utils_1.exists)(androidApiLevel) && androidApiLevel === 28) {
        ...
        if (enableCompactSched(androidApiLevel)) {
            const compact = new protos_1.FtraceConfig.CompactSchedConfig();
    ...
    if (targetInfo.targetType === 'WINDOWS' ||
        ...
        ds.config.etwConfig = new protos_1.EtwConfig();
        const kernelFlags = [];
        if (uiCfg.etwCSwitch) {
            kernelFlags.push(protos_1.EtwConfig.KernelFlag.CSWITCH);
        }
        if (uiCfg.etwThreadState) {
            kernelFlags.push(protos_1.EtwConfig.KernelFlag.DISPATCHER);
        }
...
//# sourceMappingURL=recording_config_utils.js.map