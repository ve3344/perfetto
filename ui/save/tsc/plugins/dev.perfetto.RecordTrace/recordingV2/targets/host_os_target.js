"use strict";
// Copyright (C) 2022 The Android Open Source Project
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//      http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
Object.defineProperty(exports, "__esModule", { value: true });
exports.HostOsTarget = void 0;
const host_os_byte_stream_1 = require("../host_os_byte_stream");
const recording_error_handling_1 = require("../recording_error_handling");
const recording_utils_1 = require("../recording_utils");
const traced_tracing_session_1 = require("../traced_tracing_session");
class HostOsTarget {
    maybeClearTarget;
    onTargetChange;
    targetType;
    name;
    websocket;
    streams = new Set();
    dataSources;
    onDisconnect = (_) => { };
    constructor(websocketUrl, maybeClearTarget, onTargetChange) {
        this.maybeClearTarget = maybeClearTarget;
        this.onTargetChange = onTargetChange;
        if ((0, recording_utils_1.isMacOs)(navigator.userAgent)) {
            this.name = 'MacOS';
            this.targetType = 'MACOS';
        }
        else if ((0, recording_utils_1.isLinux)(navigator.userAgent)) {
            this.name = 'Linux';
            this.targetType = 'LINUX';
        }
        else {
            throw new recording_error_handling_1.RecordingError('Host OS target created on an unsupported operating system.');
        }
        this.websocket = new WebSocket(websocketUrl);
        this.websocket.onclose = this.onClose.bind(this);
        // 'onError' gets called when the websocketURL where the UI tries to connect
        // is disallowed by the Content Security Policy. In this case, we disconnect
        // the target.
        this.websocket.onerror = this.disconnect.bind(this);
    }
    getInfo() {
        return {
            targetType: this.targetType,
            name: this.name,
            dataSources: this.dataSources || [],
        };
    }
    canCreateTracingSession() {
        return true;
    }
    async createTracingSession(tracingSessionListener) {
        this.onDisconnect = tracingSessionListener.onDisconnect;
        const osStream = await host_os_byte_stream_1.HostOsByteStream.create(this.getUrl());
        this.streams.add(osStream);
        const tracingSession = new traced_tracing_session_1.TracedTracingSession(osStream, tracingSessionListener);
        await tracingSession.initConnection();
        if (!this.dataSources) {
            this.dataSources = await tracingSession.queryServiceState();
            this.onTargetChange();
        }
        return tracingSession;
    }
    // Starts a tracing session in order to fetch data sources from the
    // device. Then, it cancels the session.
    async fetchTargetInfo(tracingSessionListener) {
        const tracingSession = await this.createTracingSession(tracingSessionListener);
        tracingSession.cancel();
    }
    async disconnect() {
        if (this.websocket.readyState === this.websocket.OPEN) {
            this.websocket.close();
            // We remove the 'onclose' callback so the 'disconnect' method doesn't get
            // executed twice.
            this.websocket.onclose = null;
        }
        for (const stream of this.streams) {
            stream.close();
        }
        // We remove the existing target from the factory if present.
        this.maybeClearTarget(this);
        // We run the onDisconnect callback in case this target is used for tracing.
        this.onDisconnect();
    }
    // We can connect to the Host OS without taking the connection away from
    // another process.
    async canConnectWithoutContention() {
        return true;
    }
    getUrl() {
        return this.websocket.url;
    }
    onClose(ev) {
        if (ev.code === recording_utils_1.WEBSOCKET_CLOSED_ABNORMALLY_CODE) {
            console.info(`It's safe to ignore the 'WebSocket connection to ${this.getUrl()} error above, if present. It occurs when ` +
                'checking the connection to the local Websocket server.');
        }
        this.disconnect();
    }
}
exports.HostOsTarget = HostOsTarget;
//# sourceMappingURL=host_os_target.js.map