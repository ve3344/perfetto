...
function bleScanQuery(condition) {
    return `
  with step1 as (
      select
          ts,
...
`;
class default_1 {
    static id = 'dev.perfetto.AndroidLongBatteryTracing';
    groups = new Map();
    addTrack(ctx, track, groupName) {
        if (groupName) {
            ...
            if (existingGroup) {
                ...
                const group = new workspace_1.TrackNode({ title: groupName, isSummary: true });
    ...
    async addSliceTrack(ctx, name, query, groupName, columns = []) {
    ...
    async addCounterTrack(ctx, name, query, groupName, options) {
        const uri = `/long_battery_tracing_${name}`;
    ...
    async addBatteryStatsState(ctx, name, track, groupName, features) {
        if (!features.has(`track.${track}`)) {
            return;
        }
        await this.addSliceTrack(ctx, name, `SELECT ts, safe_dur AS dur, value_name AS name
    FROM android_battery_stats_state
    WHERE track_name = "${track}"`, groupName);
    }
    async addBatteryStatsEvent(ctx, name, track, groupName, features) {
        if (!features.has(`track.${track}`)) {
            return;
        }
        await this.addSliceTrack(ctx, name, `SELECT ts, safe_dur AS dur, str_value AS name
    FROM android_battery_stats_event_slices
    WHERE track_name = "${track}"`, groupName);
    }
    async addDeviceState(ctx, features) {
        if (!features.has('track.battery_stats.*')) {
            return;
        }
        const query = (name, track) => this.addBatteryStatsEvent(ctx, name, track, undefined, features);
        const e = ctx.engine;
        ...
        await this.addSliceTrack(ctx, 'Device State: Suspend / resume', SUSPEND_RESUME);
        ...
        await this.addSliceTrack(ctx, 'Device State: Long wakelocks', `SELECT
        ...
        if (features.has('atom.thermal_throttling_severity_state_changed')) {
            await this.addSliceTrack(ctx, 'Device State: Thermal throttling', THERMAL_THROTTLING);
        }
    }
    async addAtomCounters(ctx) {
        const e = ctx.engine;
        try {
        ...
        const counters = await e.query(`select distinct ui_group, ui_name, ui_unit, counter_name
       from atom_counters
        ...
        for (; countersIt.valid(); countersIt.next()) {
            ...
            const opts = unit === '%'
                ...
                : unit !== undefined
                    ? { unit }
                    : undefined;
            await this.addCounterTrack(ctx, countersIt.ui_name, `select ts, ${unit === '%' ? 100.0 : 1.0} * counter_value as value
         from atom_counters
         where counter_name = '${countersIt.counter_name}'`, countersIt.ui_group, opts);
        }
    }
    async addAtomSlices(ctx) {
        const e = ctx.engine;
        try {
            await e.query(`INCLUDE PERFETTO MODULE
            google3.wireless.android.telemetry.trace_extractor.modules.atom_counters_slices`);
        ...
        const sliceTracks = await e.query(`select distinct ui_group, ui_name, atom, field
       from atom_slices
       where ui_name is not null
       order by 1, 2, 3, 4`);
        ...
        for (const [atom, args] of fields) {
            ...
            await this.addSliceTrack(ctx, tracks.get(atom).ui_name, `select ts, dur, slice_name as name, ${args.map((a) => argSql(a)).join(', ')}
         from atom_slices
         where atom = '${atom}'
         group by ts, dur, name`, tracks.get(atom).ui_group, args.map((a) => safeArg(a)));
        }
    }
    async addNetworkSummary(ctx, features) {
        ...
        await this.addSliceTrack(ctx, 'Default network', DEFAULT_NETWORK, groupName);
        ...
        if (features.has('net.wifi')) {
            await this.addCounterTrack(ctx, 'Wifi total bytes', `select ts, sum(value) as value from network_summary where dev_type = 'wifi' group by 1`, groupName, { yDisplay: 'log', yRangeSharingKey: 'net_bytes', unit: 'byte' });
            const result = await e.query(`select pkg, sum(value) from network_summary where dev_type='wifi' group by 1 order by 2 desc limit 10`);
            const it = result.iter({ pkg: 'str' });
            for (; it.valid(); it.next()) {
                await this.addCounterTrack(ctx, `Top wifi: ${it.pkg}`, `select ts, value from network_summary where dev_type = 'wifi' and pkg = '${it.pkg}'`, groupName, { yDisplay: 'log', yRangeSharingKey: 'net_bytes', unit: 'byte' });
            }
        }
        this.addBatteryStatsState(ctx, 'Wifi interface', 'battery_stats.wifi_radio', groupName, features);
        this.addBatteryStatsState(ctx, 'Wifi supplicant state', 'battery_stats.wifi_suppl', groupName, features);
        this.addBatteryStatsState(ctx, 'Wifi strength', 'battery_stats.wifi_signal_strength', groupName, features);
        if (features.has('net.modem')) {
            await this.addCounterTrack(ctx, 'Modem total bytes', `select ts, sum(value) as value from network_summary where dev_type = 'modem' group by 1`, groupName, { yDisplay: 'log', yRangeSharingKey: 'net_bytes', unit: 'byte' });
            const result = await e.query(`select pkg, sum(value) from network_summary where dev_type='modem' group by 1 order by 2 desc limit 10`);
            const it = result.iter({ pkg: 'str' });
            for (; it.valid(); it.next()) {
                await this.addCounterTrack(ctx, `Top modem: ${it.pkg}`, `select ts, value from network_summary where dev_type = 'modem' and pkg = '${it.pkg}'`, groupName, { yDisplay: 'log', yRangeSharingKey: 'net_bytes', unit: 'byte' });
            }
        }
        this.addBatteryStatsState(ctx, 'Cellular interface', 'battery_stats.mobile_radio', groupName, features);
        await this.addSliceTrack(ctx, 'Cellular connection', `select ts, dur, name from radio_transport`, groupName);
        this.addBatteryStatsState(ctx, 'Cellular strength', 'battery_stats.phone_signal_strength', groupName, features);
    }
    async addModemDetail(ctx, features) {
    ...
    async addModemRil(ctx, groupName) {
        const rilStrength = async (band, value) => await this.addSliceTrack(ctx, `Modem signal strength ${band} ${value}`, `SELECT ts, dur, name FROM RilScreenOn WHERE band_name = '${band}' AND value_name = '${value}'`, groupName);
        const e = ctx.engine;
        ...
        await this.addSliceTrack(ctx, 'Modem channel config', MODEM_RIL_CHANNELS, groupName);
        await this.addSliceTrack(ctx, 'Modem cell reselection', MODEM_CELL_RESELECTION, groupName, ['raw_ril']);
    }
    async addModemTeaData(ctx, groupName) {
        const e = ctx.engine;
        try {
            await e.query(`INCLUDE PERFETTO MODULE
        ...
        const counters = await e.query(`select distinct name from pixel_modem_counters`);
        const countersIt = counters.iter({ name: 'str' });
        for (; countersIt.valid(); countersIt.next()) {
            await this.addCounterTrack(ctx, countersIt.name, `select ts, value from pixel_modem_counters where name = '${countersIt.name}'`, groupName);
        }
        const slices = await e.query(`select distinct track_name from pixel_modem_slices`);
        const slicesIt = slices.iter({ track_name: 'str' });
        for (; slicesIt.valid(); slicesIt.next()) {
            await this.addSliceTrack(ctx, slicesIt.track_name, `select ts, dur, slice_name as name from pixel_modem_slices
            where track_name = '${slicesIt.track_name}'`, groupName);
        }
    }
    async addKernelWakelocks(ctx, features) {
        ...
        for (; it.valid(); it.next()) {
            await this.addCounterTrack(ctx, it.wakelock_name, `select ts, dur, value from kernel_wakelocks where wakelock_name = "${it.wakelock_name}"`, groupName, { yRangeSharingKey: 'kernel_wakelock', unit: '%' });
        }
    }
    async addWakeups(ctx, features) {
      ...
      having sum_dur > 600e9`);
        ...
        for (; it.valid(); it.next()) {
            labelOther = true;
            await this.addSliceTrack(ctx, `Wakeup ${it.item}`, `${sqlPrefix} where item="${it.item}"`, groupName, WAKEUPS_COLUMNS);
            items.push(it.item);
        }
        await this.addSliceTrack(ctx, labelOther ? 'Other wakeups' : 'Wakeups', `${sqlPrefix} where item not in ('${items.join("','")}')`, groupName, WAKEUPS_COLUMNS);
    }
    async addHighCpu(ctx, features) {
        ...
        const result = await e.query(`select distinct pkg, cluster from high_cpu where value > 10 order by 1, 2`);
        const it = result.iter({ pkg: 'str', cluster: 'str' });
        for (; it.valid(); it.next()) {
            await this.addCounterTrack(ctx, `CPU (${it.cluster}): ${it.pkg}`, `select ts, value from high_cpu where pkg = "${it.pkg}" and cluster="${it.cluster}"`, groupName, { yOverrideMaximum: 100, unit: '%' });
        }
    }
    async addBluetooth(ctx, features) {
        if (!Array.from(features.values()).some((f) => f.startsWith('atom.bluetooth_') || f.startsWith('atom.ble_'))) {
            return;
        }
        const groupName = 'Bluetooth';
        await this.addSliceTrack(ctx, 'BLE Scans (opportunistic)', bleScanQuery('opportunistic'), groupName);
        await this.addSliceTrack(ctx, 'BLE Scans (filtered)', bleScanQuery('filtered'), groupName);
        await this.addSliceTrack(ctx, 'BLE Scans (unfiltered)', bleScanQuery('not filtered'), groupName);
        ...
        await this.addSliceTrack(ctx, 'Link-level Events', BT_LINK_LEVEL_EVENTS, groupName, BT_LINK_LEVEL_EVENTS_COLUMNS);
        await this.addSliceTrack(ctx, 'A2DP Audio', BT_A2DP_AUDIO, groupName);
        await this.addSliceTrack(ctx, 'Bytes Transferred (L2CAP/RFCOMM)', BT_BYTES, groupName);
        await ctx.engine.query(BT_ACTIVITY);
        await this.addCounterTrack(ctx, 'ACL Classic Active Count', 'select ts, dur, acl_active_count as value from bt_activity', groupName);
        await this.addCounterTrack(ctx, 'ACL Classic Sniff Count', 'select ts, dur, acl_sniff_count as value from bt_activity', groupName);
        await this.addCounterTrack(ctx, 'ACL BLE Count', 'select ts, dur, acl_ble_count as value from bt_activity', groupName);
        await this.addCounterTrack(ctx, 'Advertising Instance Count', 'select ts, dur, advertising_count as value from bt_activity', groupName);
        await this.addCounterTrack(ctx, 'LE Scan Duty Cycle Maximum', 'select ts, dur, le_scan_duty_cycle as value from bt_activity', groupName, { unit: '%' });
        await this.addSliceTrack(ctx, 'Inquiry Active', "select ts, dur, 'Active' as name from bt_activity where inquiry_active", groupName);
        await this.addSliceTrack(ctx, 'SCO Active', "select ts, dur, 'Active' as name from bt_activity where sco_active", groupName);
        await this.addSliceTrack(ctx, 'A2DP Active', "select ts, dur, 'Active' as name from bt_activity where a2dp_active", groupName);
        await this.addSliceTrack(ctx, 'LE Audio Active', "select ts, dur, 'Active' as name from bt_activity where le_audio_active", groupName);
        await this.addCounterTrack(ctx, 'Controller Idle Time', 'select ts, dur, controller_idle_pct as value from bt_activity', groupName, { yRangeSharingKey: 'bt_controller_time', unit: '%' });
        await this.addCounterTrack(ctx, 'Controller TX Time', 'select ts, dur, controller_tx_pct as value from bt_activity', groupName, { yRangeSharingKey: 'bt_controller_time', unit: '%' });
        await this.addCounterTrack(ctx, 'Controller RX Time', 'select ts, dur, controller_rx_pct as value from bt_activity', groupName, { yRangeSharingKey: 'bt_controller_time', unit: '%' });
        await this.addSliceTrack(ctx, 'Quality reports', BT_QUALITY_REPORTS, groupName, BT_QUALITY_REPORTS_COLUMNS);
        await this.addSliceTrack(ctx, 'RSSI Reports', BT_RSSI_REPORTS, groupName, BT_RSSI_REPORTS_COLUMNS);
        await this.addSliceTrack(ctx, 'HAL Crashes', BT_HAL_CRASHES, groupName, BT_HAL_CRASHES_COLUMNS);
        await this.addSliceTrack(ctx, 'Code Path Counter', BT_CODE_PATH_COUNTER, groupName, BT_CODE_PATH_COUNTER_COLUMNS);
    }
    async addContainedTraces(ctx, containedTraces) {
        ...
        for (const [subscription, traces] of bySubscription) {
            await this.addSliceTrack(ctx, subscription, traces
                .map((t) => `SELECT
        ...
        `)
                .join(' UNION ALL '), 'Other traces', ['link']);
        }
    }
    async findFeatures(e) {
        const features = new Set();
        const addFeatures = async (q) => {
    ...
    async addTracks(ctx) {
        const features = await this.findFeatures(ctx.engine);
