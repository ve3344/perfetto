...
exports.getInputScrollDeltas = getInputScrollDeltas;
exports.getPresentedScrollDeltas = getPresentedScrollDeltas;
exports.getPredictorJankDeltas = getPredictorJankDeltas;
exports.getJankIntervals = getJankIntervals;
exports.buildScrollOffsetsGraph = buildScrollOffsetsGraph;
...
async function getInputScrollDeltas(engine, scrollId) {
  ...
  `);
    const it = queryResult.iter({
        ...
        scrollUpdateId: query_result_1.NUM,
        deltaY: query_result_1.NUM,
        offsetY: query_result_1.NUM,
    });
    const deltas = [];
...
async function getPresentedScrollDeltas(engine, scrollId) {
    const queryResult = await engine.query(`
    INCLUDE PERFETTO MODULE chrome.scroll_jank.scroll_offsets;
...
async function getPredictorJankDeltas(engine, scrollId) {
    const queryResult = await engine.query(`
  ...
  `);
    const it = queryResult.iter({
        ...
        deltaY: query_result_1.NUM,
        offsetY: query_result_1.NUM,
        predictorJank: query_result_1.NUM,
    });
    const deltas = [];
    let offset = 0;
    for (; it.valid(); it.next()) {
...
async function getJankIntervals(engine, startTs, dur) {
    const queryResult = await engine.query(`
...
function buildScrollOffsetsGraph(inputDeltas, presentedDeltas, predictorDeltas, jankIntervals) {
    ...
    const filteredPresentedDeltas = presentedDeltas.filter((item) => {
        for (let i = 0; i < predictorDeltas.length; i++) {
            const predictorDelta = predictorDeltas[i];
            if (predictorDelta.ts == item.ts &&
    ...
    const presentedData = buildOffsetData(filteredPresentedDeltas, PRESENTED_CATEGORY);
    const predictorData = buildOffsetData(predictorDeltas, PRESENTED_JANKY_CATEGORY);
    const jankData = buildJankLayerData(jankIntervals);
    return (0, mithril_1.default)(vega_view_1.VegaView, {
        spec: `
{
...
function buildOffsetData(deltas, category) {
    const plotData = [];
...
function buildJankLayerData(janks) {
    let dataJsonString = '';
...
//# sourceMappingURL=scroll_delta_graph.js.map