...
class default_1 {
    ...
    async onTraceLoad(ctx) {
        ...
        ctx.commands.registerCommand({
            ...
            callback: async () => {
                const name = await this.ctx.omnibox.prompt('Give a name to the pinned set of tracks');
                if (name) {
        ...
        ctx.commands.registerCommand({
            ...
            callback: async () => {
                ...
                const res = await this.ctx.omnibox.prompt('Select name of set of pinned tracks to restore', {
                    values: tracksByName,
                    getName: (x) => x.name,
                });
                if (res) {
        ...
        ctx.commands.registerCommand({
            ...
            callback: async () => {
                ...
                const tracks = await this.ctx.omnibox.prompt('Select name of set of pinned tracks to export', {
                    values: tracksByName,
                    getName: (x) => x.name,
                });
                if (tracks) {
        ...
        ctx.commands.registerCommand({
            ...
            callback: async () => {
                ...
                (0, logging_1.assertIsInstance)(files, HTMLInputElement).click();
            },
        });
    }
    restoreTracks(tracks) {
        ...
        tracks.forEach((trackToRestore) => {
            ...
            if (foundTrack) {
                ...
                console.warn('[RestorePinnedTracks] No track found that matches', trackToRestore);
            }
        });
    }
    getCurrentPinnedTracks() {
        return this.ctx.workspace.pinnedTracks.map((track) => this.toSavedTrack(track));
    }
    findMatchingTrack(localTracks, savedTrack) {
        let mostSimilarTrack = undefined;
        let mostSimilarTrackDifferenceScore = 0;
        for (let i = 0; i < localTracks.length; i++) {
            const localTrack = localTracks[i];
            const differenceScore = this.calculateSimilarityScore(localTrack.savedTrack, savedTrack);
    ...
    calculateSimilarityScore(track1, track2) {
        ...
        else if (this.removeNumbers(track1.trackName) ===
        ...
        else if (this.removeNumbers(track1.groupName) ===
    ...
    removeNumbers(inputString) {
        return inputString?.replace(/\d+/g, '');
    }
    toSavedTrack(track) {
        let trackDescriptor = undefined;
...
function addOrReplaceNamedPinnedTracks({ name, tracks }) {
    const savedState = getSavedState();
    const rawTracksByName = savedState?.tracksByName ?? [];
    const tracksByNameMap = new Map(rawTracksByName.map((x) => [x.name, x.tracks]));
...
function groupName(track) {
    const parent = track.parent;
    if (parent instanceof workspace_1.TrackNode) {
        return parent.title;
    }
    return undefined;
}
const SAVED_PINNED_TRACK_SCHEMA = zod_1.z
    .object({
    ...
    isMainThread: zod_1.z.boolean(),
})
    .readonly();
const SAVED_NAMED_PINNED_TRACKS_SCHEMA = zod_1.z
    .object({
    name: zod_1.z.string(),
    tracks: zod_1.z.array(SAVED_PINNED_TRACK_SCHEMA).readonly(),
})
    .readonly();
const SAVED_STATE_SCHEMA = zod_1.z
    .object({
    tracks: zod_1.z.array(SAVED_PINNED_TRACK_SCHEMA).optional().readonly(),
    tracksByName: zod_1.z
