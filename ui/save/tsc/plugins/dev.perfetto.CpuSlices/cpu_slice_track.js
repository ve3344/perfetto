...
const sched_details_tab_1 = require("./sched_details_tab");
...
class CpuSliceTrack {
    ...
    constructor(trace, uri, cpu, threads) {
        this.trace = trace;
        this.uri = uri;
        this.cpu = cpu;
    ...
    `);
      ...
      where cpu = ${this.cpu} and utid != 0
    `);
        this.lastRowId = it.firstRow({ lastRowId: query_result_1.NUM }).lastRowId;
    }
    getDataset() {
    ...
    `);
        ...
        const it = queryRes.iter({
            tsQ: query_result_1.LONG,
            tsEndQ: query_result_1.LONG,
            utid: query_result_1.NUM,
            id: query_result_1.NUM,
            isIncomplete: query_result_1.NUM,
            isRealtime: query_result_1.NUM,
        });
    ...
    async onDestroy() {
        await this.trace.engine.tryQuery(`drop table if exists cpu_slice_${this.trackUuid}`);
    ...
    renderSlices({ ctx, timescale, size, visibleWindow }, data) {
        (0, logging_1.assertTrue)(data.startQs.length === data.endQs.length);
        (0, logging_1.assertTrue)(data.startQs.length === data.utids.length);
        ...
        const [, rawEndIdx] = (0, binary_search_1.searchSegment)(data.startQs, endTime);
        const endIdx = rawEndIdx === -1 ? data.startQs.length : rawEndIdx;
        for (let i = startIdx; i < endIdx; i++) {
            const tStart = time_1.Time.fromRaw(data.startQs[i]);
            let tEnd = time_1.Time.fromRaw(data.endQs[i]);
            ...
            if (data.flags[i] & CPU_SLICE_FLAGS_INCOMPLETE) {
                (0, canvas_utils_1.drawIncompleteSlice)(ctx, rectStart, MARGIN_TOP, rectWidth, RECT_HEIGHT);
            }
            else {
            ...
            title = (0, string_utils_1.cropText)(title, charWidth, visibleWidth);
            subTitle = (0, string_utils_1.cropText)(subTitle, charWidth, visibleWidth);
        ...
        if (selection.kind === 'track_event') {
            if (selection.trackUri === this.uri) {
                const [startIndex, endIndex] = (0, binary_search_1.searchEq)(data.ids, selection.eventId);
                if (startIndex !== endIndex) {
                    const tStart = time_1.Time.fromRaw(data.startQs[startIndex]);
                    const tEnd = time_1.Time.fromRaw(data.endQs[startIndex]);
                    const utid = data.utids[startIndex];
                    const color = (0, colorizer_1.colorForThread)(this.threads.get(utid));
                    const rectStart = timescale.timeToPx(tStart);
                    const rectEnd = timescale.timeToPx(tEnd);
                    const rectWidth = Math.max(1, rectEnd - rectStart);
                    ...
                    if (selection.wakeupTs) {
                        ...
                        (0, canvas_utils_1.drawDoubleHeadedArrow)(ctx, wakeupPos, MARGIN_TOP + RECT_HEIGHT, latencyWidth, latencyWidth >= 20);
    ...
    onMouseMove({ x, y, timescale }) {
        ...
        for (let i = 0; i < data.startQs.length; i++) {
            const tStart = time_1.Time.fromRaw(data.startQs[i]);
            const tEnd = time_1.Time.fromRaw(data.endQs[i]);
            const utid = data.utids[i];
        ...
        const threadInfo = (0, utils_1.exists)(hoveredUtid) && this.threads.get(hoveredUtid);
        // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
    ...
    onMouseClick({ x, timescale }) {
        ...
        const index = (0, binary_search_1.search)(data.startQs, time.toTime());
    ...
    async getSelectionDetails(eventId) {
    ...
    detailsPanel() {
        return new sched_details_tab_1.SchedSliceDetailsPanel(this.trace, this.threads);
    }
}
exports.CpuSliceTrack = CpuSliceTrack;
...
function getHatchedPattern(mainCtx) {
    const mctx = mainCtx;
    if (mctx.sliceHatchedPattern !== undefined)
        return mctx.sliceHatchedPattern;
    const canvas = document.createElement('canvas');
    const SIZE = 8;
    canvas.width = canvas.height = SIZE;
    const ctx = (0, logging_1.assertExists)(canvas.getContext('2d'));
    ...
    ctx.lineTo(SIZE, 0);
    ctx.stroke();
    mctx.sliceHatchedPattern = (0, logging_1.assertExists)(mctx.createPattern(canvas, 'repeat'));
    return mctx.sliceHatchedPattern;
}
//# sourceMappingURL=cpu_slice_track.js.map