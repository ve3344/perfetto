...
class CpuFreqTrack {
    ...
    async onCreate() {
        ...
        if (this.config.idleTrackId === undefined) {
          ...
          `));
            ...
            this.trash.use(await (0, sql_utils_1.createView)(this.trace.engine, `raw_freq_${this.trackUuid}`, `
            select ts, dur, value as freqValue
            from experimental_counter_dur c
            where track_id = ${this.config.freqTrackId}
          `));
            this.trash.use(await (0, sql_utils_1.createView)(this.trace.engine, `raw_idle_${this.trackUuid}`, `
            select
              ts,
              dur,
    ...
    `);
        ...
        (0, logging_1.assertTrue)(freqRows == idleRows);
        const data = {
            start,
            end,
            resolution,
        ...
        const freqIt = freqResult.iter({
            ...
            minFreq: query_result_1.NUM,
            maxFreq: query_result_1.NUM,
            lastFreq: query_result_1.NUM,
        });
        const idleIt = idleResult.iter({
            lastIdle: query_result_1.NUM,
        });
    ...
    render({ ctx, size, timescale, visibleWindow }) {
        ...
        (0, logging_1.assertTrue)(data.timestamps.length === data.lastFreqKHz.length);
        (0, logging_1.assertTrue)(data.timestamps.length === data.minFreqKHz.length);
        (0, logging_1.assertTrue)(data.timestamps.length === data.maxFreqKHz.length);
        (0, logging_1.assertTrue)(data.timestamps.length === data.lastIdleValues.length);
        ...
        const color = (0, colorizer_1.colorForCpu)(this.config.cpu);
        let saturation = 45;
        ...
        const [rawStartIdx] = (0, binary_search_1.searchSegment)(data.timestamps, start);
        const startIdx = rawStartIdx === -1 ? 0 : rawStartIdx;
        const [, rawEndIdx] = (0, binary_search_1.searchSegment)(data.timestamps, end);
        const endIdx = rawEndIdx === -1 ? data.timestamps.length : rawEndIdx;
        // Draw the CPU frequency graph.
        {
            ctx.beginPath();
            const timestamp = time_1.Time.fromRaw(data.timestamps[startIdx]);
            ctx.moveTo(Math.max(calculateX(timestamp), 0), zeroY);
            let lastDrawnY = zeroY;
            for (let i = startIdx; i < endIdx; i++) {
                const timestamp = time_1.Time.fromRaw(data.timestamps[i]);
    ...
    onMouseMove({ x, y, timescale }) {
        ...
        this.hoveredTs =
            left === -1 ? undefined : time_1.Time.fromRaw(data.timestamps[left]);
        this.hoveredTsEnd =
...
exports.CpuFreqTrack = CpuFreqTrack;
//# sourceMappingURL=cpu_freq_track.js.map