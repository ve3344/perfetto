...
exports.addJankCUJDebugTrack = addJankCUJDebugTrack;
 ...
 */
function addJankCUJDebugTrack(ctx, trackName, cujNames) {
    const jankCujTrackConfig = generateJankCujTrackConfig(cujNames);
    (0, debug_tracks_1.addDebugSliceTrack)({ trace: ctx, title: trackName, ...jankCujTrackConfig });
}
 ...
 */
function generateJankCujTrackConfig(cujNames = []) {
    ...
    const filterCuj = cujNamesList?.length > 0
...
class default_1 {
    static id = 'dev.perfetto.AndroidCujs';
    async onTraceLoad(ctx) {
        ...
        ctx.commands.registerCommand({
            ...
            callback: () => {
                ctx.engine.query(JANK_CUJ_QUERY_PRECONDITIONS).then(() => (0, query_result_tab_1.addQueryResultsTab)(ctx, {
        ...
        ctx.commands.registerCommand({
            ...
            callback: () => {
                ctx.engine.query(JANK_CUJ_QUERY_PRECONDITIONS).then(() => (0, debug_tracks_1.addDebugSliceTrack)({
                    trace: ctx,
                    data: {
