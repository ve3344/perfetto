...
class default_1 {
    static id = 'dev.perfetto.AndroidPerf';
    async addAppProcessStartsDebugTrack(ctx, reason, sliceName) {
        const sliceColumns = [
            'id',
            'ts',
            'dur',
    ...
    async onTraceLoad(ctx) {
        ctx.commands.registerCommand({
            ...
            callback: () => (0, query_result_tab_1.addQueryResultsTab)(ctx, {
        ...
        ctx.commands.registerCommand({
            ...
            callback: async () => {
                await ctx.engine.query(`INCLUDE PERFETTO MODULE android.app_process_starts;`);
        ...
        ctx.commands.registerCommand({
          ...
          `);
                    ...
                    event = await ctx.omnibox.prompt('Choose a ftrace event...', ftraceEvents);
                    if (event === undefined) {
                        return;
                    }
                }
          ...
          `);
                    const args = [];
                    const it = result.iter({ key: query_result_1.STR });
                    for (; it.valid(); it.next()) {
                        args.push(it.key);
                    }
                    value = await ctx.omnibox.prompt('Choose a argument as counter value...', args);
                    if (value === undefined) {
                        return;
                    }
                    filter = await ctx.omnibox.prompt('Choose a argument as pivot key...', args);
                    if (filter === undefined) {
                        return;
                    }
                }
                if (filterValue === undefined) {
                    filterValue = await ctx.omnibox.prompt('List the target pivot values (separate by comma) to present\n' +
                        'ex1: 123,456 \n' +
                        'ex2: "task_name1","task_name2"\n');
                    if (filterValue === null)
                        return;
                }
                await (0, debug_tracks_1.addPivotedTracks)(ctx, {
                    sqlSource: `
              SELECT
                ts,
              ...
              FROM ftrace_event
                ...
                }, event + '#' + value + '@' + filter, 'pivot', async (ctx, data, trackName) => (0, debug_tracks_1.addDebugCounterTrack)({
                    trace: ctx,
                    data,
                    title: trackName,
                }));
