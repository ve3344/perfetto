...
describe('workspace', () => {
    test('getNodeByKey', () => {
        ...
        const track = new workspace_1.TrackNode({ id: 'foo' });
        workspace.addChildLast(track);
        expect(workspace.getTrackById('foo')).toEqual(track);
    });
    test('getNodeByKey', () => {
        const track = new workspace_1.TrackNode({ id: 'bar' });
    ...
    test('findTrackByUri()', () => {
        const workspace = new workspace_1.Workspace();
        const group = new workspace_1.TrackNode();
        const track = new workspace_1.TrackNode({ uri: 'foo' });
        group.addChildLast(track);
        // Add group to workspace
        workspace.addChildLast(group);
    ...
    test('findClosestVisibleAncestor()', () => {
        const child = new workspace_1.TrackNode();
        child.expand(); // Expanding the child node should have no effect
        const parent = new workspace_1.TrackNode();
        parent.expand();
...
describe('TrackNode.addChildInOrder', () => {
    let container;
    beforeEach(() => {
        container = new workspace_1.TrackNode();
    });
    test('inserts a child into an empty container', () => {
        const child = new workspace_1.TrackNode({ id: 'track1' });
    ...
    test('inserts a child with a lower sortOrder before an existing child', () => {
        const child1 = new workspace_1.TrackNode({ sortOrder: 10 });
        const child2 = new workspace_1.TrackNode({ sortOrder: 5 });
    ...
    test('inserts a child with a higher sortOrder after an existing child', () => {
        const child1 = new workspace_1.TrackNode({ sortOrder: 5 });
        const child2 = new workspace_1.TrackNode({ sortOrder: 10 });
        container.addChildInOrder(child1);
        container.addChildInOrder(child2);
        expect(container.children).toHaveLength(2);
    ...
    test('inserts multiple children and maintains order', () => {
        const child1 = new workspace_1.TrackNode({ sortOrder: 15 });
        const child2 = new workspace_1.TrackNode({ sortOrder: 10 });
        const child3 = new workspace_1.TrackNode({ sortOrder: 20 });
        container.addChildInOrder(child1);
        container.addChildInOrder(child2);
    ...
    test('inserts a child with undefined sortOrder as 0', () => {
        ...
        const child2 = new workspace_1.TrackNode();
        container.addChildInOrder(child1);
        container.addChildInOrder(child2);
        expect(container.children).toHaveLength(2);
...
test('TrackNode::flatTracksOrdered', () => {
    const root = new workspace_1.TrackNode();
    const removeme = new workspace_1.TrackNode({ id: 'removeme' });
    root.addChildFirst(removeme);
    const foo = new workspace_1.TrackNode({ id: 'foo' });
    root.addChildLast(foo);
    foo.addChildLast(new workspace_1.TrackNode({ id: 'fooBar' })); // <-- Note this one is added as a child of foo
    const bar = new workspace_1.TrackNode({ id: 'bar' });
    root.addChildLast(bar);
    root.addChildFirst(new workspace_1.TrackNode({ id: 'baz' })); // <- Note this one is added first so should appear before the others in flatTracks
...
test('TrackNode::flatTracks', () => {
    ...
    root.addChildLast(foo);
    foo.addChildLast(new workspace_1.TrackNode({ id: 'fooBar' })); // <-- Note this one is added as a child of foo
    root.addChildLast(new workspace_1.TrackNode({ id: 'bar' }));
    root.addChildFirst(new workspace_1.TrackNode({ id: 'baz' })); // <- Note this one is added first so should appear before the others in flatTracks
    root.removeChild(removeme);
    expect(root.flatTracks.map(({ id }) => id)).toEqual(expect.arrayContaining(['baz', 'foo', 'fooBar', 'bar']));
    expect(root.flatTracks.length).toBe(4);
});
test('TrackNode::clone', () => {
    const root = new workspace_1.TrackNode();
    const childA = new workspace_1.TrackNode();
    root.addChildLast(childA);
    const childB = new workspace_1.TrackNode();
...
//# sourceMappingURL=workspace_unittest.js.map