...
exports.getOrCreateUserInteractionGroup = getOrCreateUserInteractionGroup;
 ...
 */
function getOrCreateUserInteractionGroup(workspace) {
...
// group by ID.
function getOrCreateGroup(workspace, id, args) {
    const group = workspace.getTrackById(id);
    if (group) {
        return group;
    }
    else {
        const group = new workspace_1.TrackNode({ id, ...args });
        workspace.addChildInOrder(group);
        return group;
    }
}
//# sourceMappingURL=standard_groups.js.map