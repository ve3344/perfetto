...
exports.QueryError = exports.LONG_NULL = exports.LONG = exports.BLOB_NULL = exports.BLOB = exports.STR_NULL = exports.NUM_NULL = exports.STR = exports.NUM = exports.UNKNOWN = void 0;
exports.decodeInt64Varint = decodeInt64Varint;
exports.createQueryResult = createQueryResult;
exports.timeFromSql = timeFromSql;
exports.durationFromSql = durationFromSql;
...
function isCompatible(actual, expected) {
    switch (actual) {
        case CellType.CELL_NULL:
            return (expected === exports.NUM_NULL ||
                expected === exports.STR_NULL ||
                expected === exports.BLOB_NULL ||
                expected === exports.LONG_NULL ||
                expected === exports.UNKNOWN);
        case CellType.CELL_VARINT:
            return (expected === exports.NUM ||
                expected === exports.NUM_NULL ||
                expected === exports.LONG ||
                expected === exports.LONG_NULL ||
                expected === exports.UNKNOWN);
        case CellType.CELL_FLOAT64:
            return expected === exports.NUM || expected === exports.NUM_NULL || expected === exports.UNKNOWN;
        case CellType.CELL_STRING:
            return expected === exports.STR || expected === exports.STR_NULL || expected === exports.UNKNOWN;
        case CellType.CELL_BLOB:
            return (expected === exports.BLOB || expected === exports.BLOB_NULL || expected === exports.UNKNOWN);
        default:
        ...
        while (reader.pos < end) {
            ...
            switch (tag >>> 3) {
                ...
                case 3: // float64_cells, a 64-bit aligned packed fixed64 buffer.
                    ...
                    if (f64Off % 8 === 0) {
                        this.float64Cells = new Float64Array(batchBytes.buffer, f64Off, f64Words);
                    }
                    else {
...
class RowIteratorImpl {
    ...
    batchIdx = -1; // The batch index within |result.batches[]|.
    batchBytes = new Uint8Array();
    columnNames = [];
    numColumns = 0;
    cellTypesEnd = -1; // -1 so the 1st next() hits tryMoveToNextBatch().
    float64Cells = new Float64Array();
    varIntReader = minimal_1.default.Reader.create(this.batchBytes);
    blobCells = [];
    stringCells = [];
    ...
    get(columnName) {
        ...
        if (res === undefined) {
            throw this.makeError(`Column '${columnName}' doesn't exist. ` +
    ...
    next() {
        ...
        while (this.nextCellTypeOff + this.numColumns > this.cellTypesEnd) {
            ...
            (0, logging_1.assertTrue)(this.nextCellTypeOff === this.cellTypesEnd || this.cellTypesEnd === -1);
    ...
    tryMoveToNextBatch() {
        ...
        for (const expectedCol of Object.keys(this.rowSpec)) {
            if (this.columnNames.indexOf(expectedCol) < 0) {
                throw this.makeError(`Column ${expectedCol} not found in the SQL result ` +
...
function timeFromSql(value) {
    if (typeof value === 'bigint') {
        return time_1.Time.fromRaw(value);
    }
    else if (typeof value === 'number') {
        return time_1.Time.fromRaw(BigInt(Math.floor(value)));
    }
    else if (value === null) {
...
function durationFromSql(value) {
    if (typeof value === 'bigint') {
        return value;
    }
    else if (typeof value === 'number') {
