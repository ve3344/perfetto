...
class default_1 {
    ...
    async onTraceLoad(trace) {
        trace.onTraceReady.addListener(() => {
            ...
            groupGlobalIostatTracks(trace, F2FS_IOSTAT_LAT_TAG, F2FS_IOSTAT_LAT_GROUP_NAME);
...
function groupGlobalIonTracks(trace) {
    const ionTracks = [];
    ...
    for (const track of ionTracks) {
        if (!memGroupNode &&
            ...
            memGroupNode = new workspace_1.TrackNode({
                uri: track.uri,
                title: track.title,
                isSummary: true,
            });
...
function groupGlobalIostatTracks(trace, tag, groupName) {
...
function groupGlobalBuddyInfoTracks(trace) {
    const devMap = new Map();
    for (const track of trace.workspace.children) {
        if (track.hasChildren)
            continue;
...
function groupFrequencyTracks(trace, groupName) {
    const group = new workspace_1.TrackNode({ title: groupName, isSummary: true });
    for (const track of trace.workspace.children) {
        if (track.hasChildren)
            continue;
        ...
        // frequency ones).
        if (track.title.endsWith('Frequency') &&
...
function groupMiscNonAllowlistedTracks(trace, groupName) {
...
function groupTracksByRegex(trace, regex, groupName) {
    const group = new workspace_1.TrackNode({ title: groupName, isSummary: true });
    for (const track of trace.workspace.children) {
        if (track.hasChildren)
            continue;
